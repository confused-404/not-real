...
u16 sign_extend(uint16_t x, int bit_count)
{
    if ((x >> (bit_count - 1)) & 1) {
        x |= (0xFFFF << bit_count);
    }
...
u16 swap16(uint16_t x)
{
    return (x << 8) | (x >> 8);
}
void update_flags(VM* vm, u16 r)
{
    if (vm->reg[r] == 0)
    {
        vm->reg[R_COND] = FL_ZRO;
...
void mem_write(VM* vm, u16 address, uint16_t val)
{
    vm->mem[address] = val;
}

u16 mem_read(VM* vm, uint16_t address)
{
    if (address == MR_KBSR)
    {
        if (check_key())
...
static void op_br(VM* vm, u16 instr)
{
...
static void op_trap(VM* vm, u16 instr)
{
...
static void op_and(VM* vm, u16 instr)
{
    u16 r0 = (instr >> 9) & 0x7;
    u16 r1 = (instr >> 6) & 0x7;
    u16 imm_flag = (instr >> 5) & 0x1;
...
static void op_not(VM* vm, u16 instr)
{
    u16 r0 = (instr >> 9) & 0x7;
    u16 r1 = (instr >> 6) & 0x7;
...
static void op_jmp(VM* vm, u16 instr)
{
    u16 r1 = (instr >> 6) & 0x7;
    vm->reg[R_PC] = vm->reg[r1];
}

static void op_jsr(VM* vm, u16 instr)
{
...
static void op_ld(VM* vm, u16 instr)
{
    u16 r0 = (instr >> 9) & 0x7;
    u16 pc_offset = sign_extend(instr & 0x1FF, 9);
...
static void op_ldi(VM* vm, u16 instr)
{
    u16 r0 = (instr >> 9) & 0x7;
    u16 pc_offset = sign_extend(instr & 0x1FF, 9);
...
static void op_ldr(VM* vm, u16 instr)
{
    u16 r0 = (instr >> 9) & 0x7;
    u16 r1 = (instr >> 6) & 0x7;
...
static void op_lea(VM* vm, u16 instr)
{
    u16 r0 = (instr >> 9) & 0x7;
    u16 pc_offset = sign_extend(instr & 0x1FF, 9);
...
static void op_st(VM* vm, u16 instr)
{
    u16 r0 = (instr >> 9) & 0x7;
    u16 pc_offset = sign_extend(instr & 0x1FF, 9);
...
static void op_sti(VM* vm, u16 instr)
{
    u16 r0 = (instr >> 9) & 0x7;
    u16 pc_offset = sign_extend(instr & 0x1FF, 9);
...
static void op_str(VM* vm, u16 instr)
{
    u16 r0 = (instr >> 9) & 0x7;
    u16 r1 = (instr >> 6) & 0x7;
    u16 offset = sign_extend(instr & 0x3F, 6);
